...
impl Persister {
    async fn set_tracked_zaps_safe<F, R>(&self, f: F) -> NostrResult<R>
    where
    ...
    pub(crate) async fn list_tracked_zaps(&self) -> NostrResult<TrackedZaps> {
    ...
    pub(crate) async fn add_tracked_zap(
        &self,
        invoice: String,
        zap_request: String,
    ) -> NostrResult<()> {
    ...
    pub(crate) async fn get_tracked_zap_raw(&self, invoice: &str) -> NostrResult<Option<String>> {
        self.set_tracked_zaps_safe(|tracked_zaps| {
    ...
    pub(crate) async fn remove_tracked_zap(
        &self,
        invoice: &str,
    ...
    pub(crate) async fn clean_expired_zaps(&self) -> NostrResult<()> {
        let now = utils::now();
        self.set_tracked_zaps_safe(|tracked_zaps| {
            let mut expired = vec![];
