...
impl<'a> EncryptionHandler<'a> {
    ...
    fn nip44_decrypt(&self, msg: &str) -> Result<String, NostrError> {
        nostr_sdk::nips::nip44::decrypt(self.our_secret, self.client_pubkey, msg).map_err(|err| {
            NostrError::Encryption {
                err: err.to_string(),
            }
        })
    }

    fn nip04_decrypt(&self, msg: &str) -> Result<String, NostrError> {
    ...
    pub(crate) fn decrypt(&self, e: &Event) -> Result<String, NostrError> {
        match e
            .tags
    ...
    pub(crate) fn nip44_encrypt(&self, msg: &str) -> Result<String, NostrError> {
        nostr_sdk::nips::nip44::encrypt(self.our_secret, self.client_pubkey, msg, Version::V2)
            .map_err(|err| NostrError::Encryption {
                err: err.to_string(),
            })
    }

    pub(crate) fn nip04_encrypt(&self, msg: &str) -> Result<String, NostrError> {
    ...
    pub(crate) fn encrypt(&self, e: &Event, msg: &str) -> Result<String, NostrError> {
        match e
            .tags
            .find(TagKind::Custom("encryption".into()))
            .is_some_and(|enc_type| enc_type.content() == Some("nip44_v2"))
